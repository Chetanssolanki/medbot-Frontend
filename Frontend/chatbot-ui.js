...
function setBotResponse(val) {
    setTimeout(function () {
        if (val.length < 1) {
            ...
            let descriptionAndPrecautions = val.split(/Description:|Precautions:/);
            
            console.log(descriptionAndPrecautions)
            let description = descriptionAndPrecautions[1].trim();
            let precautions = descriptionAndPrecautions[2].trim();
            var br = `<div class='bot-msg'><img class='bot-img' src ='${botLogoPath}' /><span class='msg'>From your given symptoms, It's most likely that you have <strong> ${descriptionAndPrecautions[0]} </strong></span></div>`;
            var BotResponse = `<div class='bot-msg'><img class='bot-img' src ='${botLogoPath}' /><span class='msg'>${description}</span></div>`;
            var br1 = `<div class='bot-msg'><img class='bot-img' src ='${botLogoPath}' /><span class='msg'> <strong> Precautions:</strong> ${precautions}. <br/> <strong> You can consult our doctors for more information and Book Appointment. </strong></span></div>`;
            $(br).appendTo('.chat-area').hide().fadeIn(1000);
            $(BotResponse).appendTo('.chat-area').hide().fadeIn(1000);
            $(br1).appendTo('.chat-area').hide().fadeIn(1000);
...
function chatbotTheme(theme) {
    ...
    const purple = {
        color: "#33d9b2",
        background: "linear-gradient(19deg, rgb(33, 212, 253) 0%, rgb(51 217 178) 100%)"
    }
